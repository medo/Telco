...
train_model <- function(data, method, grid){
  ...
  model <- train(data %>% dplyr::select(-TARGET,-CONTRACT_KEY), (data %>% dplyr::select(TARGET))[,1], method=method, trControl = fitControl, tuneGrid = grid, metric="ROC",  preProc = c("center", "scale"))
}
...
prepare_dataset <- function(data) {
  ...
  ret_data <- ret_data %>% dplyr::select(-HANDSET_NAME, -X)
  ...
  ret_data <- ret_data %>% dplyr::select(-GENDER.F)
...
```{r}
m <- train_model(train_transformed_data %>% sample_n(20000) , "gbm", NULL)
