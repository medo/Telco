...
fitControl2 <- trainControl(method = "boot",
                           number = 25,
                           savePredictions = "final",
                           classProbs = TRUE,
                           summaryFunction= twoClassSummary,
...
train_model <- function(data){
  ...
  models <- caretList(
    x=(data %>% dplyr::select(-TARGET,-CONTRACT_KEY)),
    y=(data %>% dplyr::select(TARGET))[,1],
    metric="ROC",
    preProcess=c( "center", "scale"),
    verbose=F,
    #methodList=c("nb", "treebag", "rf", "xgbTree", "gbm"),
    methodList=c("nb", "treebag", "rf"),
...
```{r}
train_data <- train_data %>% sample_n(20000)
```
...
prepare_dataset <- function(data, is_train) {
  ...
  # Adding the expected point regression
  #predict_sixth_month <- function(x1,x2,x3,x4,x5){
  #  
  #  #print(x1,x2,x3)
  #  df <- data.frame(
  #  idx= c(1,2,3,4,5),
  ...
  #}
  #
  #ret_data <- ret_data %>% lapply(function(x){
  #  print(x %>% class)
  #  
  #})
  ...
  descrCor <- cor(ret_data %>% dplyr::select(-one_of(c("TARGET"))))
  ...
  ret_data <- ret_data[, -findLinearCombos(ret_data %>% dplyr::select(-one_of(c("TARGET"))))$remove]
...
```{r}
ens <- train_model(train_transformed_data)
models <- ens$models
models %>% sapply(function(x){
  auc(as.numeric(x$pred$obs), as.numeric(x$pred$pred))
})
print("The ensambled models :")
auc(as.numeric(ens$ens_model$pred$obs), as.numeric(ens$ens_model$pred$pred))

#m <- train_model(train_transformed_data , "gbm")
