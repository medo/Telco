...
train_model <- function(data, method, grid){
  ...
  model <- train(data %>% dplyr::select(-TARGET,-CONTRACT_KEY), (data %>% dplyr::select(TARGET))[,1], method=method, trControl = fitControl, tuneGrid = grid, metric="ROC", verbose=F, preProcess=c( "center", "scale"))
}
...
evaluate_on_all_models <- function(data) {
  ...
  return(c("nb", "treebag", "rf", "xgbTree", "gbm", "rf") %>% lapply(function(algo){
    print(algo)
